...
// 获取到用户的唯一的openid
var senderopenid = decodeURI(location.href.split('?')[1].split('&')[0].split('=')[1]);
var senderimg = decodeURI(location.href.split('?')[1].split('&')[1].split('=')[1]);
console.log("senderopenid :"+senderopenid);
console.log("senderimg :"+senderimg);
var useropenid = localStorage.getItem("openid");
var userimg = localStorage.getItem("userimg");
$("#avatar").attr("src",senderimg);
// 为了不让重复调用微信的获取token调用。
var guard = localStorage.getItem("guard"); 
var search = location.href.split("?")[1];
if(guard){
        if(useropenid && userimg){
            console.log("直接进入sharecontent页面");
            alert("直接进入sharecontent页面");
            var encodeOpenid = encodeURI(useropenid);
            window.location.href="http://www.npxfxn.cn/zqweb/views/sharecontent.jsp?openid="+useropenid+"&senderopenid="+senderopenid;
        }else{
...
function getConnectWechat(){
    ...
    var redirect_url = encodeURI(location.href);
...
function AccessCode() {
    ...
    var successCallback = function(data) {
	    ...
	    var openid = encodeURI(JSON.stringify(data.detail.openid));
        var img = encodeURI(JSON.stringify(data.detail.headimgurl));
        $("#btn").attr('href','http://www.npxfxn.cn/zqweb/views/sharecontent.jsp?openid='+encodeURI(openid)+"&senderopenid="+encodeURI(senderopenid)+"&img="+encodeURI(senderimg));
    }
